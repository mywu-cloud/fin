...
# ---------------------------------------------------------------------------
# TWSE industry code → Chinese name (固定對照表)
# ---------------------------------------------------------------------------
...
async def _fetch_twse_industry_map(client: httpx.AsyncClient) -> Dict[str, str]:
    ...
    try:
        ...
        for item in data:
            ...
            industry_raw = str(item.get("產業別", "") or item.get("IndustryCode", "")).strip()
            if not code:
                continue
            # Numeric code → Chinese name; already Chinese → use as-is
            if industry_raw and industry_raw.lstrip("0123456789") == "":
        ...
        try:
            ...
            for item in data:
                ...
                if code and ind:
                    result[code] = ind
            if result:
                logger.info("TPEx industry map: %d entries (from %s)", len(result), url)
                return result
        except Exception as e:
    ...
    try:
        ...
        for item in data:
            ...
            if not code or not name:
                continue
            try:
                close = float(str(item.get("ClosingPrice", "")).replace(",", "")) or None
    ...
    try:
        ...
        for item in data:
            ...
            if not code or not name:
                continue
            try:
                close = float(str(item.get("Close", "")).replace(",", "")) or None
            except (ValueError, TypeError):
                close = None
            industry = (
...
async def _upsert_revenues(rows: List[Dict]) -> None:
    ...
    for r in rows:
        try:
            ...
            cumulative_revenue: Optional[int] = cum or None
    ...
    # Use index_elements instead of constraint= (SQLite dialect)
...
async def run_sync(full: bool = False) -> None:
    ...
    # Use a single long-lived client for the entire sync
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        # 3. Revenue sync — all requests inside the same client context
        if full:
        ...
        # gather inside the async with client block — client stays open
